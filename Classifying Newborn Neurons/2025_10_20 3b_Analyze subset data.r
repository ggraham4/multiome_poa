...
observed_100 = read_parquet("Classifying Newborn Neurons/output/2025_10_20_100_restarts_observed.parquet")
...
shuffled_100 =read_parquet("Classifying Newborn Neurons/output/2025_10_20_100_restarts_shuffled.parquet")
#Same as above, but with shuffled labels (null expectation)

again_observed_100 = read_parquet("Classifying Newborn Neurons/output/2025_10_20_again_100_restarts_observed.parquet")
...
again_shuffled_100 = read_parquet("Classifying Newborn Neurons/output/2025_10_20_again_100_restarts_shuffled.parquet")
...
# two sided p-value 
(1-sum(again_shuffled_100[,-1]%>%as.matrix()%>%as.numeric()%>%na.omit()< means['9'])/length(again_shuffled_100[,-1]%>%as.matrix()))/2
#0
...
ggplot(data_for_plot, aes(x = percentage))+
  ...
  geom_histogram(data = data_for_plot2, aes(x = percentage, fill = cluster))+
  geom_text(data = data_for_plot2, aes(x = percentage, y =1, label = cluster))+
  labs(x = 'Percentage of Cells in 1_2 Predicted to Cluster', y = 'Count')
