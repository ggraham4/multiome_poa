...
KT_deg_function <- function(object = obj, 
  ...
  meta_data <- data.frame(
    ...
    KT = object@meta.data$Log_11KT[object@meta.data[[clustering]] == cluster]
  )
  ...
  deg_output2$KT_q.value <- p.adjust(deg_output2$KT_p.value, 'fdr', nrow(deg_output2))
...
for(i in 0:31){
  ...
  write.csv(kt_data,paste0('X:/Fish Lab/Experiments/sex change single nuc POA/Seurat Outputs/012625 KT DEGs no Status/KT_results_cluster_',i,'.csv'))
  end <- Sys.time()
  print(end-start)
}
