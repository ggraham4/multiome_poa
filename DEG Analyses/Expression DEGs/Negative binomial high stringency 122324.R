...
neg.bin.mult <- function(obj, clustering = 'harmony.wnn_res0.4_clusters', cluster, n_cores = detectCores() - 1) {
...
for (i in 31:0) {
  ...
  output <- neg.bin.mult(obj,
                         ...
                         i)
  write.csv(output, paste0('/Volumes/jrhodes/Fish Lab/Experiments/sex change single nuc POA/Seurat Outputs/122324 Neg Bin with Doms/cluster_', i, '.csv'))
}
#openxlsx::write.xlsx(results_cluster0, paste0('/Volumes/jrhodes/Fish Lab/Experiments/sex change single nuc POA/Seurat Outputs/072024 Neg Bin with Doms/cluster_', '0', '.xlsx'))



#Example usage:
clust_5 <- neg.bin.mult(obj,
             'harmony.wnn_res0.4_clusters',
             31)
