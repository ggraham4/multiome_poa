...
data_for_plot <- data_defined%>%
  group_by(cluster, class)%>%
  summarize(n_degs = n())%>%
  na.omit()

ggplot(data_for_plot, aes(x = cluster, y = n_degs, fill = class))+
  geom_bar(stat = 'identity',position = 'stack')+
  scale_x_continuous(breaks = c(0:26))+
  scale_y_continuous(breaks = c(0:60))
#spikes in radial glia, poa_4, 11 is microglia

DimPlot(obj, label = T)
DimPlot(obj, label = T, group.by ='gabe_celltype_ids')

DEGs_6 = data_defined%>%
  subset(cluster == 6 & class != 'NA')
### LOC111565832 SIN3-HDAC complex-associated factor-like
