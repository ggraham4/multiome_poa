...
change_Mass_function <- function(object = obj, 
  ...
  counts <- t(as.matrix(object@assays$RNA$data[, object@meta.data[[clustering]] == cluster & (object@meta.data$Status == 'D'|object@meta.data$Status =='E')]))
  
  filtered_cols_matrix <- counts[,which(colSums(counts) != 0)]
  
  meta_data <- data.frame(
    cells = rownames(object@meta.data[object@meta.data[[clustering]] == cluster & ((object@meta.data$Status == 'D'|object@meta.data$Status =='E')|object@meta.data$Status =='E'),]),
    Status = object@meta.data$Status[object@meta.data[[clustering]] == cluster & (object@meta.data$Status == 'D'|object@meta.data$Status =='E')],
    individual =object@meta.data$individual[object@meta.data[[clustering]] == cluster & (object@meta.data$Status == 'D'|object@meta.data$Status =='E')],
    change_Mass = object@meta.data$Change_Mass[object@meta.data[[clustering]] == cluster &(object@meta.data$Status == 'D'|object@meta.data$Status =='E')]
  )
...
for(i in 31:0){
  ...
  write.csv(change_Mass_data ,paste0('DEG Outputs/022625 Change Mass DEGs w expanded/results_cluster_',i,'.csv'))
  end <- Sys.time()
  print(end-start)
}
