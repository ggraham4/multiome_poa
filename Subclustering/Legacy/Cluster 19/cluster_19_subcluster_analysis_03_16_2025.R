...
cluster_19_data$status <- ifelse(cluster_19_data$status == 'D', 'I', cluster_19_data$status)
cluster_19_data$status <- ifelse(cluster_19_data$status == 'E', 'LI', cluster_19_data$status)

cluster_19_data$status <- factor(cluster_19_data$status, levels = c('M','I',"LI",'NF','F'))
 ...
 cell_counts$prop[cell_counts$Var2 == 'I'] = cell_counts$Freq[cell_counts$Var2 == 'I']/ nrow(cluster_19@meta.data[cluster_19@meta.data$Status == 'I',])
 ...
 cell_counts$prop[cell_counts$Var2 == 'LI'] = cell_counts$Freq[cell_counts$Var2 == 'LI']/ nrow(cluster_19@meta.data[cluster_19@meta.data$Status == 'LI',])
 ...
 cell_counts$Var2 <- factor(cell_counts$Var2, levels = c('M','I',"LI",'NF','F'))
 ...
 linear_model <- lmer(cyto~prim_neuro + cluster+ (1|individual), data = subset(cluster_19_data, status %in% c('M','I','F')))
 ...
 linear_model_subcluster <- lmer(cyto~ status*cluster+ (1|individual), data = subset(cluster_19_data, status %in% c('M','I','F')))
 car::Anova(linear_model_subcluster, type ='III')
  ...
  data_cyto_plot <- subset(cluster_19_data, status %in% c('M','I','F'))%>%
    na.omit()%>%
...
sub_cells = cluster_19@meta.data%>%
 ...
 hsd17b14_data <- data.frame(hsd17b14_expression = clust_19_1_data,
                              individual = cluster_19$individual[cluster_19$sub =='19_1'],
 ...
 hsd17b14_data$Sex <- ifelse( hsd17b14_data$Sex == "D",'I', hsd17b14_data$Sex)
 hsd17b14_data$Sex <- ifelse( hsd17b14_data$Sex == "E",'LI', hsd17b14_data$Sex)
 
 hsd17b14_data$Sex <- factor(hsd17b14_data$Sex, levels = c('M','I','LI',"NF",'F'))
 ...
 pgr_data <- data.frame(pgr_expression = clust_19_1_data_pgr,
                             individual = cluster_19$individual[cluster_19$sub =='19_2'],
 ...
 pgr_data$Sex <- ifelse( pgr_data$Sex == "D",'I', pgr_data$Sex)
 pgr_data$Sex <- ifelse( pgr_data$Sex == "E",'LI', pgr_data$Sex)
 
 pgr_data$Sex <- factor(pgr_data$Sex, levels = c('M','I','LI',"NF",'F')) 
