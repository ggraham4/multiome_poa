# cluster 6_3 beta binomial IEG analysis
...
RNA_data <- sub_6@assays$RNA$data%>%as.matrix()
ieg_rows_rna <- RNA_data[which(rownames(RNA_data) %in% iegs_),]
ieg_scores <- colSums(ieg_rows_rna>0)

sub_6$ieg_scores = ieg_scores
### beta binomial regression
library(glmmTMB)
library(performance)

data_6_3 = subset(sub_6@meta.data, sub.cluster =='6_3')
...
out_dat = data.frame()
for(i in unique(sub_6$sub.cluster)){
  
  temp_data = subset(sub_6@meta.data, sub.cluster ==i & Status != 'NRM')
