...
obj_subclustered_6 <- FindSubCluster(obj, c(6), 'harmony.wsnn')
obj_subclustered_1 <- FindSubCluster(obj, c(1), 'harmony.wsnn')
...
#get mean counts in a cluster
mean_counts <- function(object, gene, cluster, clustering){
  message(cluster)
  # Get the indices of the cells in the specified cluster
  cells_in_cluster <- which(object@meta.data[[clustering]] == cluster)
  
  # Get the column index for the gene
  gene_index <- which(rownames(object@assays$RNA$counts) == gene)
  
  # Pull the values directly (gene = row, cells = columns)
  gene_counts <- object@assays$RNA$counts[gene_index, cells_in_cluster]
  
  # Return the max
  return(mean(gene_counts))
}
mean_counts(obj_subclustered_6, 'cckbrb','6_1', 'sub.cluster')
mean_counts(obj_subclustered_1, 'LOC111577263','1_3', 'sub.cluster')
mean_counts(obj_subclustered_6, 'mpz','2', 'sub.cluster')

values = obj@assays$RNA$counts%>%as.matrix()
hist(log(values[values>0]))
quantile((values[values>0]))
