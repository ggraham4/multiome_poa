#### ATAC HEATMAP for CLUSTER 6 POA ####
...
obj_6_only = subset(obj, res0.8_50nn_40PC_45LSI == 6)
DefaultAssay(obj_6_only) =='ATAC'

dars = read.csv("Collaboration/all_clusters_DARs_peak_level_classified_with_support.csv")

dars_6 =subset(dars, cluster_id ==6)
dars_6

# --- Step 1: Get gene list from cluster 6 DARS ---
dars_to_plot <- dars_6$peak
dars_to_plot <- dars_to_plot[dars_to_plot %in% rownames(obj_6_only@assays$ATAC)]

# --- Step 2: Extract normalized expression and compute z-scores per status ---
expr_matrix <- GetAssayData(obj_6_only, assay = "ATAC", layer = "data")[dars_to_plot, ]
...
p = pheatmap(
  ...
  fontsize_col = 11,
  border_color = NA,
  main = "6_POA_Mixed DARs — Mean Z-Score by Phase"
)

ggsave(plot = p,
       file = paste0('dar_matrix_6.svg'),
       device = "svg",
       units = "in",
       width = 3.69,
       height = 2.13,
