...
obj_1_only = subset(obj, final_clusters == 1)
degs = read.csv('~/Desktop/multiome_poa/DEG Outputs/FINAL degs classified w singular.csv')
degs_1 = subset(degs, cluster == 1)



# --- Step 1: Get gene list from cluster 1 DEGs ---
genes_to_plot <- degs_1$gene
genes_to_plot <- genes_to_plot[genes_to_plot %in% rownames(obj_1_only)]

# --- Step 2: Extract normalized expression and compute z-scores per status ---
expr_matrix <- GetAssayData(obj_1_only, assay = "RNA", layer = "data")[genes_to_plot, ]
...
p = pheatmap(
  ...
  fontsize_col = 11,
  border_color = NA,
  main = "1_RGCs DEGs — Mean Z-Score by Phase"
)

   ggsave(plot = p,
       file = paste0('deg_matrix_1.svg'),
       device = "svg",
       units = "in",
       width = 7.5,
       height = 3.8,
       path = paste0("Manuscript/Plots/Manuscript v1.2.1"))
