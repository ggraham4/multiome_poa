...
signif_cells = dat$cluster[!is.na(dat$sig)]
for(i in signif_cells){
p = proportion_plotter(joint, i,
            d_m_p = dat$d_m_p.value[dat$cluster == i],
            f_m_p = dat$f_m_p.value[dat$cluster == i],
            d_f_p = dat$d_f_p.value[dat$cluster == i]) +
  labs(y = '% of Cells')

ggsave(plot = p,
       file = paste0("clust_",i,"_prop.svg"),
       device = "svg",
       units = "in",
       width = 1.5,
       height = 1.5,
       path = '/Users/ggraham/Desktop/multiome_poa/Manuscript/Plots/Manuscript v1.2.1/prop_cells_all')
...
signif_neurons = dat_neuron$cluster[!is.na(dat_neuron$sig)]
for(i in signif_neurons){
p = proportion_plotter(joint, i,
            d_m_p = dat_neuron$d_m_p.value[dat_neuron$cluster == i],
            f_m_p = dat_neuron$f_m_p.value[dat_neuron$cluster == i],
            d_f_p = dat_neuron$d_f_p.value[dat_neuron$cluster == i]) +
  labs(y = '% of Neurons')
...
ggsave(plot = p,
       ...
       width = 1.5,
       height = 1.5,
       path = '/Users/ggraham/Desktop/multiome_poa/Manuscript/Plots/Manuscript v1.2.1/prop_neurons_all')
