...
# identify cells within each "cluster" (neuron type) to connect
clusters <- list(WhichCells(GT_neurons, expression = CHAT > 0 | slc18a3b > 0), #ach
                 WhichCells(GT_neurons, expression = th > 0 | th2 > 0 | slc6a3), #da
                 WhichCells(GT_neurons, expression = dbh > 0 | slc6a2 > 0), #ne
                 WhichCells(GT_neurons, expression = TPH1 > 0 | tph2 > 0 | slc6a4a > 0), #sero
                 WhichCells(GT_neurons, expression = hdc > 0), #hist
                 WhichCells(GT_neurons, expression = ENSAPEG00000007875 > 0), #gnrh
                 WhichCells(GT_neurons, expression = ENSAPEG00000019310 > 0), #gnih
                 WhichCells(GT_neurons, expression = ENSAPEG00000002502 > 0), #kiss
                 WhichCells(GT_neurons, expression = oxt > 0), #oxt
                 WhichCells(GT_neurons, expression = avp > 0), #avt
                 WhichCells(GT_neurons, expression = galn > 0), #gal
                 WhichCells(GT_neurons, expression = NPY > 0 | npy > 0), #npy
                 WhichCells(GT_neurons, expression = ghrh > 0), #ghrh
                 WhichCells(GT_neurons, expression = ccka > 0 | cckb > 0), #cck
                 WhichCells(GT_neurons, expression = ADCYAP1 > 0 | ADCYAP1 > 0), #pacap
                 WhichCells(GT_neurons, expression = vip > 0), #vip
                 WhichCells(GT_neurons, expression = tac1 > 0 | tac3a > 0) #tac
                 )

# identify cells positive for each receptor type
expmatrix <- GetAssayData(GT_neurons, slot = "data")

ach_r <- genes_interest$geneID[genes_interest$system == "acetylcholine" & genes_interest$genetype == "receptor"]
ach_r_cells <- c()
for (i in ach_r) {
  geneindex <- which(rownames(expmatrix) == i)
  cellnames <- colnames(expmatrix)[expmatrix[geneindex,] > 0]
  ach_r_cells <- append(ach_r_cells, cellnames)
}
ach_r_cells <- unique(ach_r_cells)
...
for (i in da_r) {
  geneindex <- which(rownames(expmatrix) == i)
  cellnames <- colnames(expmatrix)[expmatrix[geneindex,] > 0]
  da_r_cells <- append(da_r_cells, cellnames)
}
da_r_cells <- unique(da_r_cells)

ne_r <- genes_interest$geneID[genes_interest$system == "norepinephrine" & genes_interest$genetype == "receptor"]
ne_r_cells <- c()
for (i in ne_r) {
  geneindex <- which(rownames(expmatrix) == i)
  cellnames <- colnames(expmatrix)[expmatrix[geneindex,] > 0]
  ne_r_cells <- append(ne_r_cells, cellnames)
}
ne_r_cells <- unique(ne_r_cells)

sero_r <- genes_interest$geneID[genes_interest$system == "serotonin" & genes_interest$genetype == "receptor"]
sero_r_cells <- c()
for (i in sero_r) {
  geneindex <- which(rownames(expmatrix) == i)
  cellnames <- colnames(expmatrix)[expmatrix[geneindex,] > 0]
  sero_r_cells <- append(sero_r_cells, cellnames)
}
sero_r_cells <- unique(sero_r_cells)

hist_r <- genes_interest$geneID[genes_interest$system == "histamine" & genes_interest$genetype == "receptor"]
hist_r_cells <- c()
for (i in hist_r) {
  geneindex <- which(rownames(expmatrix) == i)
  cellnames <- colnames(expmatrix)[expmatrix[geneindex,] > 0]
  hist_r_cells <- append(hist_r_cells, cellnames)
}
hist_r_cells <- unique(hist_r_cells)

gnrh_r_cells <- WhichCells(GT_neurons, expression = gnrhr1 > 0 | ENSAPEG00000000902 > 0 | gnrhr4 > 0)
gnih_r_cells <- WhichCells(GT_neurons, expression = NPFFR2 > 0 | npffr2b > 0 | npffr1l2 > 0)
kiss_r_cells <- WhichCells(GT_neurons, expression = ENSAPEG00000019191 > 0)
oxt_r_cells <- WhichCells(GT_neurons, expression = oxtr > 0 | oxtrl > 0)
avt_r_cells <- WhichCells(GT_neurons, expression = AVPR1A > 0 | avpr1ab > 0 | AVPR2 > 0 | avpr2aa > 0 | avpr2l > 0)
gal_r_cells <- WhichCells(GT_neurons, expression = galr1a > 0 | galr2a > 0)
npy_r_cells <- WhichCells(GT_neurons, expression = NPY2R > 0 | npy2r > 0 | npy7r > 0 | npy8ar > 0 | npy8br > 0)
ghrh_r_cells <- WhichCells(GT_neurons, expression = GHRHR > 0 | ghrhra > 0 | ghrhrb > 0 | ghrhr2 > 0)
cck_r_cells <- WhichCells(GT_neurons, expression = CCKAR > 0 | ENSAPEG00000000774 > 0)
pacap_r_cells <- WhichCells(GT_neurons, expression = adcyap1r1a > 0)
vip_r_cells <- WhichCells(GT_neurons, expression = vipr1a > 0 | vipr1b > 0 | vipr2 > 0)
tac_r_cells <- WhichCells(GT_neurons, expression = tacr1b > 0 | tacr2 > 0 | TACR3 > 0)
ar_r_cells <- WhichCells(GT_neurons, expression = ar > 0 | ENSAPEG00000010255 > 0)
er_r_cells <- WhichCells(GT_neurons, expression = esr1 > 0 | esr2a > 0 | esr2b > 0)
pgr_r_cells <- WhichCells(GT_neurons, expression = pgr > 0)

receptors <- list(ach_r_cells,
                  da_r_cells,
                  ne_r_cells,
                  sero_r_cells,
                  hist_r_cells,
...
matrix_pct <- data.frame(matrix(nrow = 0, ncol = 0))
for (i in clusters) {
  rowout <- c()
  for (j in receptors) {
    overlap <- ((sum(i %in% j)) / length(i)) * 100
    rowout <- append(rowout, overlap)
  }
  matrix_pct <- rbind(matrix_pct, rowout)
}


celltypes <- read_xlsx("network_tables.xlsx", sheet = 1)
receptors <- read_xlsx("network_tables.xlsx", sheet = 2)

rownames(matrix_pct) <- celltypes$label
colnames(matrix_pct) <- receptors$label


# preliminary heatmap
heatmap(as.matrix(matrix_pct))

matrix_pct_long <- matrix_pct %>%
  rowid_to_column("id") %>%
  pivot_longer(cols = 2:21, names_to = "receptor", values_to = "percent")


# brief interlude to make barcharts that summarize the results here
barchart_gnrh <- 
  matrix_pct_long %>%
  filter(id == 6) %>%
  ggplot(aes(x = reorder(receptor, -percent), y = percent)) +
  geom_col() +
  theme_bw() +
...
edges <- matrix_pct_long %>%
  rename(to = id) %>%
  left_join(nodes, by = c("receptor" = "label")) %>% # receptor in edges matched to label in nodes
  rename(from = id)
edges <- select(edges, to, from, percent)
...
#plot(igraphtest, edge.arrow.size = 0.4)
...
# make solo plots for GnRH, DA, etc.
edges_gnrh <- filter(edges, to == 6)
ggraphtest <- tbl_graph(nodes = nodes, 
                        edges = edges_gnrh, 
                        directed = TRUE)
ggraph(ggraphtest) + 
