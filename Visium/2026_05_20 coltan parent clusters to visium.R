...
obj_balanced_neuron = subset(obj_balanced, clusters20 %notin% non_neuronal)
...
vis_neuron_sub <- TransferData(
  ...
  query            = vis_neuron,
  reference        = obj_balanced_neuron,
  refdata          = list(transferred_cluster = "clusters20"),
...
mul_to_region = pheatmap(mul[,order(colnames(mul)%>%as.numeric())],
         ...
         treeheight_row = 0, 
         main = 'Percent of Each Region Parent Clusters')  


ggsave(plot = mul_to_region,
       file = "pakrer_clusters20_to_region.svg",
       device = "svg",
       units = "in",
       width = 3.5,
       height = 3.5,
...
obj_balanced_neuron@meta.data$top_anatomical_region <- 
  cluster_top_region[as.character(obj_balanced_neuron@meta.data$clusters20)]
...
obj_balanced_neuron@meta.data$second_anatomical_region <- 
  cluster_second_region[as.character(obj_balanced_neuron@meta.data$clusters20)]
