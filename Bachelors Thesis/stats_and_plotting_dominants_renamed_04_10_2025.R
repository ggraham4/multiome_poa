...
data$Status <- ifelse(data$Status == 'D', 'I', data$Status)
data$Status <- ifelse(data$Status == 'S', 'IP', data$Status)
data$Status <- ifelse(data$Status == 'E', 'LI', data$Status)
data$Status <- ifelse(data$Status == 'EP', 'LIP', data$Status)
...
behavior_plot <- ggplot(data, aes(x = Status_dummy, y = Behaviors_Day_2))+
  ...
  theme_classic()+
  geom_signif(xmin = c(1), xmax = c(4), y_position = c(max(data$Behaviors_Day_2)*1.2), annotation =c("*"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(3), xmax = c(4), y_position = c(max(data$Behaviors_Day_2)*1.02), annotation =c("*"), color = "black", tip_length = c(0,0), textsize=6)+
  labs(x  ='Sex', y = 'Parental Care Behaviors')+
  ylim(c(0, max(data$Behaviors_Day_2)*1.3))+
...
ggsave(plot = behavior_plot,
       file = "behavior_plot_renamed.svg",
       device = "svg",
       units = "in",
       width = 2.5,
       height = 2.5,
       path = "Bachelors Thesis/Plots_dominants_renamed/Figure 1")
...
time_plot <- ggplot(data, aes(x = Status_dummy, y = Time_Day_2))+
  ...
  geom_signif(xmin = c(1), xmax = c(2), y_position = c(max(data$Time_Day_2)*1.05), annotation =c("*"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(1), xmax = c(4), y_position = c(max(data$Time_Day_2)*1.25), annotation =c("*"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(3), xmax = c(4), y_position = c(max(data$Time_Day_2)*1.05), annotation =c("**"), color = "black", tip_length = c(0,0), textsize=6)+
  theme(axis.text.x = element_text(angle = -45, vjust = 1, hjust=0), legend.position = 'none')+
  ylim(c(0, max(data$Time_Day_2)*1.4))+
...
sd(na.omit(data$Log_11KT[data$Status == 'M']))
sd(na.omit(data$Log_11KT[data$Status == 'D']))
...
kt_plot <- ggplot(kt_data, aes(x = Status_dummy, y = Log_11KT))+
  ...
  geom_signif(xmin = c(1), xmax = c(1.8), y_position = c(max(kt_data$Log_11KT)*1.05), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(1), xmax = c(4), y_position = c(max(kt_data$Log_11KT)*1.5), annotation =c("*"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(2), xmax = c(4), y_position = c(max(kt_data$Log_11KT)*1.3), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(2.2), xmax = c(3), y_position = c(max(kt_data$Log_11KT)*1.05), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  theme(axis.text.x = element_text(angle = -45, vjust = 1, hjust=0), legend.position = 'none')+
...
test_plot <- ggplot(test_data, aes(x = Status_dummy, y = Percent_Testicular))+
  ...
  geom_signif(xmin = c(1), xmax = c(2), y_position = c(max(test_data$Percent_Testicular)*1.02), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(1), xmax = c(3), y_position = c(max(test_data$Percent_Testicular)*1.25), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  theme(axis.text.x = element_text(angle = -45, vjust = 1, hjust=0), legend.position = 'none')+
  ylim(c(0, max(test_data$Percent_Testicular)*1.4))+
  scale_color_manual(values =c('#0099ff', '#cc9933','#669900','#00cc66','#00cccc','#ff66cc'))+
  scale_fill_manual(values =c('#0099ff', '#cc9933','#669900','#00cc66','#00cccc', '#ff66cc'))+
theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10), axis.title.x = element_text(size = 12), axis.title.y = element_text(size = 12))
...
ov_plot <- ggplot(ov_data, aes(x = Status_dummy, y = Percent_Ovarian))+
  ...
  geom_signif(xmin = c(1), xmax = c(2), y_position = c(max(ov_data$Percent_Ovarian)*1.02), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(1), xmax = c(3), y_position = c(max(ov_data$Percent_Ovarian)*1.25), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  theme(axis.text.x = element_text(angle = -45, vjust = 1, hjust=0), legend.position = 'none')+
  ylim(c(0, max(ov_data$Percent_Ovarian)*1.4))+
  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10), axis.title.x = element_text(size = 12), axis.title.y = element_text(size = 12))+
...
ev_2.5x_model <- lm(Log10_Volume~Status_dummy+length_final_cm, data = data)
...
residual_model <- lm(Log10_Volume~length_final_cm, data = data)
residual_model$residuals

car::Anova(ev_2.5x_model, type =3)
pairs(emmeans(ev_2.5x_model, 'Status_dummy'), adjust = 'none')

ev_2.5x_data$residual_volume = residual_model$residuals

ev_2.5x_plot <- ggplot(ev_2.5x_data, aes(x = Status_dummy, y = residual_volume))+
  geom_boxplot(data = subset(ev_2.5x_data, !is.na(Status)),aes(group = Status, fill = Status, , color = Status), alpha = 0.25, outlier.shape = NA)+
  geom_point(size = 2, color = 'black', shape = 1, position = position_dodge2(0.5))+
  theme_classic()+
  labs(x  ='Sex', y = 'Corrected Volume (px)')+
  geom_signif(xmin = c(1), xmax = c(3), y_position = c(max(ev_2.5x_data$residual_volume)*1.5), annotation =c("***"), color = "black", tip_length = c(0,0), textsize=6)+
  geom_signif(xmin = c(1), xmax = c(2), y_position = c(max(ev_2.5x_data$residual_volume)*1.02), annotation =c("*"), color = "black", tip_length = c(0,0), textsize=6)+
  
    theme(axis.text.x = element_text(angle = -45, vjust = 1, hjust=0), legend.position = 'none')+
  ylim(c(-0.6, max(ev_2.5x_data$residual_volume)*1.7))+
  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10), axis.title.x = element_text(size = 12), axis.title.y = element_text(size = 12))+
  scale_color_manual(values =c('#0099ff', '#cc9933','#669900','#00cc66','#00cccc','#ff66cc'))+
  scale_fill_manual(values =c('#0099ff', '#cc9933','#669900','#00cc66','#00cccc', '#ff66cc'))
ev_2.5x_plot
...
obj_subset_2 = obj[,
                   obj@meta.data$Status != 'NRM']
nrow(obj_subset_2@meta.data)
mean(obj_subset_2@meta.data$nFeature_RNA)
mean(obj_subset_2@meta.data$nCount_RNA)
mean(obj_subset_2@meta.data$nFeature_ATAC)
mean(obj_subset_2@meta.data$nCount_ATAC)

mean(obj_subset_2@meta.data$UMI)
