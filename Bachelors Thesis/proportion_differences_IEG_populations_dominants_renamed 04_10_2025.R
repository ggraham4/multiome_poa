...
ieg_data_individual_level2$Status <- factor(ieg_data_individual_level2$Status, levels = c('M','D','E','NF','F'))
...
clust_10$Status <- as.character(clust_10$Status)
clust_10$Status <- ifelse(clust_10$Status == 'D',"I",clust_10$Status)
clust_10$Status <- ifelse(clust_10$Status == 'E',"LI",clust_10$Status)

clust_10$Status <- factor(clust_10$Status, levels =c('M','I','LI','NF','F') )
...
clust_10_plot <- ggplot(clust_10, aes(x = Status,  y = prop_pos))+
  ...
  geom_signif(xmin = 1, xmax = 5, y_position = 0.7, annotation = '*', color = 'black', textsize = 7, tip_length =c(0,0))+
  ylim(0,1.2*0.7)
clust_10_plot
...
clust_13$Status <- as.character(clust_13$Status)
clust_13$Status <- ifelse(clust_13$Status == 'D',"I",clust_13$Status)
clust_13$Status <- ifelse(clust_13$Status == 'E',"LI",clust_13$Status)

clust_13$Status <- factor(clust_13$Status, levels =c('M','I','LI','NF','F') )
...
clust_13_plot <- ggplot(clust_13, aes(x = Status,  y = prop_pos))+
  ...
  theme(legend.position = 'none')+
  theme(axis.text.y = element_text(size = 10), axis.text.x = element_text(size = 10), axis.title.x = element_text(size = 12), axis.title.y = element_text(size = 12))+
  geom_signif(xmin = 1, xmax = 5, y_position = 1.2, annotation = '*', color = 'black', textsize = 7, tip_length =c(0,0))+
  ylim(0,1.2*1.2)
clust_13_plot
...
clust_21$Status <- as.character(clust_21$Status)
clust_21$Status <- ifelse(clust_21$Status == 'D',"I",clust_21$Status)
clust_21$Status <- ifelse(clust_21$Status == 'E',"LI",clust_21$Status)

clust_21$Status <- factor(clust_21$Status, levels =c('M','I','LI','NF','F') )
...
clust_21_plot <- ggplot(clust_21, aes(x = Status,  y = prop_pos))+
  ...
  geom_signif(xmin = 2.1, xmax = 5, y_position = .8, annotation = '*', color = 'black', textsize = 7, tip_length =c(0,0))+
  ylim(0,1.2*.8)
clust_21_plot
