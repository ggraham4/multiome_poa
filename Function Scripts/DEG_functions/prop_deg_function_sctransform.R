prop_deg_function_sctransform <- function(object = obj, cluster = 19, clustering = 'harmony.wnn_res0.4_clusters'){
  start <- Sys.time()
  library(lme4)
  library(dplyr)
  library(parallel)
  ...
  counts <- t(as.matrix(object@assays$SCT$data[, object@meta.data[[clustering]] == cluster & (object@meta.data$Status == "M" | object@meta.data$Status == "F" | object@meta.data$Status == "D")]))
...
saveRDS(prop_deg_function_sctransform, 'Functions/DEG_functions/prop_deg_function_sctransform.rds')
