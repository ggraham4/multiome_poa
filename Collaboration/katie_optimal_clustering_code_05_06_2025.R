...
nemo <- readRDS("C:/Users/Gabe/Downloads/new_nemo_POAonly_clusters_5.4.25_v1.rds")
...
pdf("A:/optimal_clustering_05_06_2025/harmony_WNN_clustering_RandIndex_nemo.orig_all_cells.pdf", width = 60, height = 60)
...
nemo@meta.data <- nemo@meta.data[ , c(1:78, which(colnames(nemo@meta.data)==max_mean))] # where XXX:XXX is the range of column numbers that correspond to the columns you want to remove

colnames(nemo@meta.data) # confirm correct columns were removed


# save final object 
saveRDS(nemo, 'A:/optimal_clustering_05_06_2025/nemo.orig_harmony.integration_all_testd_clusters.rds')


################# END OF OPTIMAL CLUSERING CODE #######################


DimPlot(nemo, group.by = max_mean,
        reduction = 'harmony_wnn.umap',
        label = T
        )

FeaturePlot(nemo, 'esr2b',
            reduction = 'harmony_wnn.umap'
            )
